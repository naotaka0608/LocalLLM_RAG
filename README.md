...
- **チャット履歴機能**: 過去の会話を保存・復元（LocalStorage使用）
- **サイドバーナビゲーション**: 左側に履歴一覧を表示、クリックで切り替え
- **ドラッグ&ドロップ対応**: ファイルを直接ドロップしてアップロード
- **ローディング表示**: アップロードと回答生成の進行状況を可視化
- **通知システム**: 操作結果をリアルタイムで通知
- **レスポンシブデザイン**: モバイル・タブレットに完全対応
- **参照元の開閉**: 回答の参照元を折りたたみ表示（デフォルトで閉じた状態）
- **コンパクトデザイン**: ヘッダーとタブを小型化し、チャットエリアを最大化
...
1. **ドキュメントのアップロード**
   ...
   - 「⚙️ 設定」タブをクリック
   - **方法1**: 「📄 ドキュメント管理」エリアにファイルをドラッグ&ドロップ
   - **方法2**: エリアをクリックしてファイル選択ダイアログから選択
   - 対応形式: PDF, TXT, MD, CSV
   - 複数ファイルの同時アップロードに対応
...
2. **質問する**
   ...
   - **RAG使用**と**クエリ拡張**のチェックボックスで機能を切り替え
   - チャットエリアに質問を入力して送信（Enterキーでも送信可能）
   - アップロードしたドキュメントの内容に基づいて回答が生成されます
   - 回答は**リアルタイムで1文字ずつ表示**されます（点滅カーソル付き）
   - 生成速度（文字/秒）、総時間、文字数が表示されます
   - 回答には参照元のファイル名とページ情報も表示されます
   - 参照元はクリックすると開閉できます（デフォルトで閉じた状態）

3. **会話履歴機能の活用**
   - **自動保持**: 過去10往復（20メッセージ）の会話を自動的に記憶
   - **文脈理解**: 代名詞（「それ」「その製品」など）を理解して回答
   - **継続的な対話**: 前の質問を参照した追加質問が可能
   ...
   **使用例:**
   ...
   ユーザー: 「それは競合と比べてどう？」  ← 「それ」がXC-2000を指すと理解
   AI: 「XC-2000の9,800円は中価格帯で...」

   ユーザー: 「その価格差の理由は？」  ← 文脈を理解
   AI: 「XC-2000は温度センサー搭載...」
   ```

4. **チャット履歴の管理**
   - 左サイドバーに過去のチャット履歴が表示されます
   - 「+ 新規チャット」ボタンで新しい会話を開始
   - 履歴項目をクリックすると過去の会話を復元
   - 履歴はブラウザのLocalStorageに自動保存されます
   - 最初のユーザーメッセージがチャットのタイトルになります
   - **モバイル**: 「新規チャット」ボタンタップでサイドバーを開閉
...
│   │   └── styles.css   # スタイルシート
│   └── js/
│       └── script.js    # JavaScript（ドラッグ&ドロップ、API通信等）
...
### v0.4.0 (2025-12-26) - 最新版
- ✅ **会話履歴保持機能（セッション管理）**
  - 過去の会話を自動的に保持（最新10往復/20メッセージ）
  - 代名詞（「それ」「その製品」など）を理解した対話
  - チャットセッションごとに会話コンテキストを管理
  - RAGと会話履歴を組み合わせた高精度な回答生成
- ✅ **リアルタイムストリーミング表示**
  - Ollama APIを直接呼び出してバッファリングを削減
  - 文字がリアルタイムで1文字ずつ表示
  - 点滅カーソル（▊）で生成中を視覚化
  - 生成速度の表示（文字/秒、総時間、文字数）
- ✅ **RAG ON/OFF機能**
  - チャット画面のチェックボックスでRAGを切り替え
  - 簡潔な説明テキスト付き（ドキュメント参照/高精度・低速）
  - クエリ拡張のON/OFF機能も追加
- ✅ **モバイル対応の改善**
  - サイドバーを折りたたみ式に変更（初期高さ50px）
  - 「新規チャット」ボタンでサイドバーを開閉
  - チャット履歴選択後、自動的にサイドバーを閉じる
  - スマートフォンでもチャット画面が見やすく
- ✅ **UI/UX改善**
  - RAG・クエリ拡張の設定をチャットヘッダーに移動
  - レスポンシブデザインの最適化
  - ストリーミング中のカーソルアニメーション

### v0.3.0 (2025-12-20)
- ✅ チャット履歴機能の追加
  - LocalStorageを使用した会話の永続化
  - サイドバーに履歴一覧を表示
  - 過去の会話の復元機能
  - 新規チャット作成機能
- ✅ UI/UXの改善
  - ヘッダーサイズの縮小（タイトル: 1.3rem、サブタイトル: 0.85rem）
  - タブボタンの最適化（縦padding: 1px、フォント: 0.9rem）
  - チャットエリアの最適化（padding、margin削減）
  - 参照元をデフォルトで閉じた状態に変更
  - メッセージとヒントボックスのフォントサイズ調整
  - より精度の高い回答を得るコツを初回メッセージに表示

### v0.2.0 (2025-12-16)
- ✅ フロントエンド構造の改善
  - HTML, CSS, JavaScriptをファイル分割
  - frontendフォルダに整理
- ✅ ドラッグ&ドロップでファイルアップロード対応
- ✅ 参照元の開閉機能追加
- ✅ モバイルレスポンシブ対応強化
  - スマートフォン・タブレット完全対応
  - モデル選択UIの最適化
- ✅ ネットワークアクセス対応
  - 同じネットワーク内の別PCからアクセス可能
  - IPアドレスの動的解決
